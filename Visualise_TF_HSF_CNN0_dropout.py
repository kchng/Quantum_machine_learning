...
def makecircles( r, d, positions_x, positions_y, probability, colour, prob_colour=0.0, dropout=False):
    add_cylinder = bpy.ops.mesh.primitive_cylinder_add
    n = 0
    for l in range(len(positions_x)) :
        if dropout and ( l == 2) or dropout and ( l == 1) :
            transparent_val = 1.00
        elif dropout :
            transparent_val = 0.25
        else :
            transparent_val = 1.00
        add_cylinder(location=(positions_x[l],positions_y[l],0.0))
        bpy.context.object.scale = (r,r,d)
        setMaterial(bpy.context.object, makeMaterial('Color', colour,(1,1,1),1, transparent=True, t_alpha = transparent_val))
        n += 1
    ...
    if probability.any() > 0 and prob_colour == 0.0 :
        ...
        for l in range(len(positions_x)) :
            ...
            add_cylinder(location=(positions_x[l],positions_y[l],probability[l]))
            bpy.context.object.scale = (r/2.,r/2.,probability[l])
            setMaterial(bpy.context.object, makeMaterial('Color', prob_colour[l],(1,1,1),1, transparent=True, t_alpha = transparent_val))
            n += 1
...
def make_neuron_connections(x_right, y_right, x_left_array, y_left_array, z_right = 0.0, z_left_array=0.0, dropout=False):
    ...
    for i in range(len(x_pivot)) :
        if dropout and ( i == 2) or dropout and ( i == 1) :
            transparent_val = 0.75
        elif dropout :
            transparent_val = 0.25
        else :
            transparent_val = 0.50
        ...
        setMaterial(bpy.context.object, makeMaterial('Color', (0.25,0.25,0.25),(1,1,1),1, transparent=True, t_alpha = transparent_val))
...
output_filename = "C:\\Users\\phoni\\Dropbox\\Temp\\fully_connected_layer1.dat" 
...
makecircles( r, d, fc1_pos_x, fc1_pos_y, output, (0.25,0.25,0.25), output_prob_colour, dropout=True)
...
# Draw the connections
for i in range(len(feature_map1_pos_x)):
    make_neuron_connections(feature_map1_pos_x[i], feature_map1_pos_y[i], fc1_pos_x, fc1_pos_y, dropout=True)
...
#output_filename = "C:\\Users\\phoni\\Dropbox\\Temp\\output.dat" 
#probability = np.loadtxt(output_filename, unpack = True)*2
probability_tmp = abs(np.random.randn((1)))
probability = np.zeros(2)
probability[0] = probability_tmp
probability[1] = 1.-probability_tmp
...
for i in range(len(output_pos_x)):
    make_neuron_connections(output_pos_x[i], output_pos_y[i], fc1_pos_x, fc1_pos_y, dropout=True)
